...
export function AnimatedAIChat() {
    ...
    return (
        <div className="flex flex-col w-full items-center justify-center bg-transparent text-foreground p-6 relative overflow-hidden">
            ...
            <div className="w-full max-w-2xl mx-auto relative">
                ...
                >
                    <div className="text-center space-y-3">
                        ...
                        >
                            <h1 className="text-3xl font-medium tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-primary to-accent pb-1">
                    ...
                    <motion.div 
                        className="relative backdrop-blur-2xl bg-card/60 rounded-2xl border border-border shadow-2xl"
                        initial={{ scale: 0.98 }}
                        animate={{ scale: 1 }}
                        ...
                        <AnimatePresence>
                            {showCommandPalette && (
                                <motion.div 
                                    ...
                                    className="absolute left-4 right-4 bottom-full mb-2 backdrop-blur-xl bg-card rounded-lg z-50 shadow-lg border border-border overflow-hidden"
                            ...
                            {attachments.length > 0 && (
                                ...
                                >
                                    {attachments.map((file, index) => (
                                        <motion.div
                                            key={index}
                                            className="flex items-center gap-2 text-xs bg-muted py-1.5 px-3 rounded-lg text-foreground"
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-2">
                        {commandSuggestions.map((suggestion, index) => (
                            <motion.button
                                key={suggestion.prefix}
                                onClick={() => selectCommandSuggestion(index)}
                                className="flex items-center gap-2 px-3 py-2 bg-card hover:bg-muted rounded-lg text-sm text-muted-foreground hover:text-foreground transition-all relative group border border-border"
                                initial={{ opacity: 0, y: 10 }}
                                animate={{ opacity: 1, y: 0 }}
            ...
            <AnimatePresence>
                {isTyping && (
                    <motion.div 
                        className="fixed bottom-8 mx-auto transform -translate-x-1/2 backdrop-blur-2xl bg-card rounded-full px-4 py-2 shadow-lg border border-border z-50"
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 20 }}
                    >
            ...
            {inputFocused && (
                <motion.div 
                    className="fixed w-[50rem] h-[50rem] rounded-full pointer-events-none z-0 opacity-[0.02] bg-gradient-to-r from-primary via-accent to-chart-2 blur-[96px]"
                    animate={{
